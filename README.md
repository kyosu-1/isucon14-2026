...
| **最高記録** | **1,218,271** | **1111x** |
| 最新構成（計測OFF・5回、うち1回は再起動試験） | 1,192,302 〜 1,218,271 | |

- 有効な回はすべて `pass=true`。WARN は nearby の「既にライド中」が 1回あたり 0〜79件（上限200）
- **再起動試験に5回合格**（3台を同時に `systemctl reboot` → 全サービス自動起動 → ベンチ）
- 約3時間で 1,159,771。その後、椅子と依頼の分布を分析してマッチングを調整し（約45分）、1,218,271
- 競技サーバーは c5.large × 3台（レギュレーションどおり）。ベンチ機は別インスタンス（c5.2xlarge）
- 計測 96回。改善の根拠はすべて `measurements/` とコミットメッセージにある
