...
| **最高記録** | **1,196,520** | **1091x** |
| 最新構成（計測OFF・3回、うち1回は再起動試験） | 1,164,393 〜 1,196,520 | |

- 有効な回はすべて `pass=true`。WARN は nearby の「既にライド中」が 1回あたり 0〜79件（上限200）
- **再起動試験に4回合格**（3台を同時に `systemctl reboot` → 全サービス自動起動 → ベンチ）
- 約3時間で 1,159,771。その後、椅子と依頼の分布を分析し、マッチングを20分調整して 1,196,520
- 競技サーバーは c5.large × 3台（レギュレーションどおり）。ベンチ機は別インスタンス（c5.2xlarge）
- 計測 80回。改善の根拠はすべて `measurements/` とコミットメッセージにある
...
- マッチングは待ちライドと空き椅子の全組を「迎車時間 + 乗車距離×(1/速度 − 1/平均速度) − 8×待ち秒数」の
  小さい順に貪欲に割り当てる（0.1秒ごと）
