...
### 07:44 マッチング以外: 椅子の追加の速さ・ベンチ機の飽和

**分かったこと（07:00 / 07:35 のベンチログと DB）**

- オーナーは売上が一定額に届くたびに椅子を追加する（ベンチログ「一定の売上が立ったためオーナーの椅子が増加します」）。
  1社の椅子は開始時の約4台から終了時の約420〜480台で、ほぼ全部がこの追加。追加数は 1〜2台/回から終盤は 169〜206台/回。
- 追加の決定（ベンチログ）と登録（chairs.created_at）を突き合わせると、登録は 1社あたり毎秒 5〜7台で頭打ち。
  決定から登録完了までの遅れが後半は 13秒まで伸び、次の追加（売上の確認）は前の回の登録が終わった ~1.5秒後に来る。
  → 1台ずつ「登録 → 稼働開始」を順に呼んでいて、その応答時間が椅子の増える速さを決めている。
- 登録・稼働開始だけ access log を取ると（20260922-074913）、負荷後半は 登録 60〜125ms + 稼働開始 40〜135ms、
  apptime ≒ reqtime（アプリ内の DB 同期待ち）。負荷終了後の 619台は 1台 2ms。
- **ベンチ機（c5.2xlarge）の CPU が負荷後半に 97%（idle 3%）で飽和している。** 競技サーバーは nginx 107% / アプリ 96% /
  MySQL 79%（上限各 200%）で余裕がある。同じ時刻から「実移動時間の不満」（累計）が 0.6% → 44.7% に跳ねる。
  序盤のライドは 1% 未満なので椅子の速度の問題ではなく、ベンチ内の椅子の動きが CPU 不足で遅れていると考えられる。

| 変更 | スコア | 判断 |
| --- | --- | --- |
| 登録・稼働開始だけ access log（計測） | 1,160,128 | 上の内訳 |
| 登録・稼働開始をメモリで応答し、DB は FIFO で後から書く | 1,226,980 / 1,187,052 / 1,177,400 / 1,191,662 | 残す。登録 32→17〜23ms、稼働した椅子 1,544 → 1,685〜1,749 台。ただし平均は変わらず（下記） |
| GOGC=400 / GOMEMLIMIT=2GiB | 1,171,600 | 戻した。GC は CPU の ~10%、アプリ CPU 101→93% に減ったがスコアは変わらず |
| 再起動試験（最終構成） | 1,211,918 | 合格 |

（3・4回目は CPU プロファイルを同時に取った回）

- 椅子は狙いどおり早く増えたが、**そのぶんベンチ機の負荷が増え、椅子の動きが遅くなって相殺された**:
  実移動時間の不満 42.6〜44.7% → 47.9〜51.5%、招待経由の新規登録 3,856 → 3,257〜3,599。
- 負荷後半の CPU プロファイル（アプリ ~150%）: syscall 21%、GC ~10%、マッチングの候補選び（全ライドのフルソート）7.5%、
  ハンガリアン法の本体 0.6%。アプリの CPU を減らしても（GOGC）スコアは動かなかったので、今の上限はベンチ機。
- ベンチ機は 02:50 に c5.xlarge → c5.2xlarge に変えている（競技サーバーは不変）。**これ以上ベンチ機は変えない**（ユーザー判断）。
  スコアはベンチ機の性能に依存するので、この環境での値として扱う。

反省: GOGC のコミットに計測記録（measurements / scores）も入れていたため、`git revert` がコンフリクトして記録の削除を
ステージしかけ、その途中の状態で deploy が走った（env.sh は戻したい状態と同じだったので実害なし）。
`git revert --abort` してファイル単位で戻した。**コードの変更と計測記録は必ず別コミットにする。**
